...
**Decisions your code already made.** The stack was chosen — by an AI, a tutorial, or past-you — whether or not anyone understood why. Walk the major ones (language, framework, database, hosting) one at a time: what each is in plain language, whether it's the popular boring choice or an unusual one — say so honestly. Then check forward, per the hard rule, and let the repo do the work: "if we tore `<framework>` out tomorrow, what in your app stops working?", "where would you look in here to prove `<database>` is actually being used?", or "what would have to change to swap it for `<alternative>`?" Never "so what does `<framework>` do for your app?" — you answered that a breath ago. Same pedagogy as greenfield planning, inverted tense. Record each in `plan.md` as inherited: understood, or honestly still fuzzy with the section where it gets revisited.
