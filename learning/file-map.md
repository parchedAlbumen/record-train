...
- backend/main.py — known (2026-08-27) — the FastAPI app: creates the `app` object uvicorn runs, and defines the routes; currently just `GET /` returning a JSON status message → \[\[fastapi-basics\]\] \[\[endpoints-and-routes\]\] \[\[decorators\]\]